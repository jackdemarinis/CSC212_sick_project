...
bool BMP::detectRed(char* filename){
    std::vector<std::pair<int,int> > z_points;
    std::pair<int,int> temp_point;
    std::pair<int,int> min_points;
    std::pair<int,int> max_points;
    min_points.first = 10000;
    min_points.second = 10000;
    max_points.first = 0;
    max_points.second = 0;
    ...
    for(int i = 0; i < og_pic.TellWidth(); i++){
        for(int j = 0; j < og_pic.TellHeight();j++){
            ...
            if(int(temp_pixel.Red) > 255 - threshold && int(temp_pixel.Green)< 0 + threshold && int(temp_pixel.Blue) < 0 + threshold){
                ...
                z_points.push_back(temp_point);
                if(temp_point.first >= max_points.first){
                    max_points.first = temp_point.first;}
                if(temp_point.second >= max_points.second){
                    max_points.second = temp_point.second;}
                if(temp_point.first <= min_points.first){
                    min_points.first = temp_point.first;}
                if(temp_point.second <= min_points.second){
                    min_points.second = temp_point.second;}
            }
        }
        //std::cout<<std::endl;
    }
    //std::cout<<og_pic.TellHeight(),

    std::ofstream outFile("points.txt");

    for(int i = 0; i < z_points.size(); i ++){
        outFile << z_points[i].first << " " << z_points[i].second << std::endl;
    }
